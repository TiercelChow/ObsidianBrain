...
impl ToolHandler for GetInspirationHandler {
    fn name(&self) -> &str { "get_inspiration" }
    fn description(&self) -> &str { "从用户的知识库中生成灵感。支持三种模式：随机概念组合、反向提问、对立观点。" }
    fn input_schema(&self) -> Value { definitions::get_inspiration_schema() }
    fn module(&self) -> &str { "inspiration" }
    ...
    async fn handle(&self, args: Value, ctx: &Arc<AppContext>) -> Result<Value, BrainError> {
        ...
        let result = ctx.inspiration_service.get_inspiration(insp_type, note_path).await?;
