...
impl ToolHandler for SaveConfigHandler {
    ...
    fn description(&self) -> &str { "保存系统配置（热更新，无需重启）" }
    ...
    async fn handle(&self, args: Value, ctx: &Arc<AppContext>) -> Result<Value, BrainError> {
        ...
        if let Some(llm_cfg) = args.get("llm") {
            ...
            if let Some(p) = llm_cfg.get("provider").and_then(|v| v.as_str()) { new_config.provider = p.to_string(); }
            if let Some(m) = llm_cfg.get("model").and_then(|v| v.as_str()) { new_config.model = m.to_string(); }
            if let Some(k) = llm_cfg.get("api_key").and_then(|v| v.as_str()) { new_config.api_key = Some(k.to_string()); }
            if let Some(k) = llm_cfg.get("api_key_env").and_then(|v| v.as_str()) { new_config.api_key_env = Some(k.to_string()); }
            if let Some(u) = llm_cfg.get("base_url").and_then(|v| v.as_str()) { new_config.base_url = Some(u.to_string()); }
            if let Some(t) = llm_cfg.get("max_tokens").and_then(|v| v.as_u64()) { new_config.max_tokens = t as u32; }
            if let Some(t) = llm_cfg.get("temperature").and_then(|v| v.as_f64()) { new_config.temperature = t; }
            ...
            match crate::infra::llm_client::LlmClientFactory::create(&new_config) {
                Ok(boxed) => {
                    ...
                    tracing::info!("LLM provider hot-reloaded: {} / {}", new_config.provider, new_config.model);
                }
                Err(e) => {
...
impl ToolHandler for VerifyLlmHandler {
    fn name(&self) -> &str { "verify_llm" }
    fn description(&self) -> &str { "验证 LLM 配置是否可用（发送测试消息）" }
    ...
    async fn handle(&self, args: Value, _ctx: &Arc<AppContext>) -> Result<Value, BrainError> {
        ...
        if let Some(p) = args.get("provider").and_then(|v| v.as_str()) { config.provider = p.to_string(); }
        if let Some(m) = args.get("model").and_then(|v| v.as_str()) { config.model = m.to_string(); }
        if let Some(k) = args.get("api_key").and_then(|v| v.as_str()) { config.api_key = Some(k.to_string()); }
        if let Some(k) = args.get("api_key_env").and_then(|v| v.as_str()) { config.api_key_env = Some(k.to_string()); }
        if let Some(u) = args.get("base_url").and_then(|v| v.as_str()) { config.base_url = Some(u.to_string()); }

        // Create provider
        let provider = crate::infra::llm_client::LlmClientFactory::create(&config)
