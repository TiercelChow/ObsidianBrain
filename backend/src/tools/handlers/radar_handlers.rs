...
impl ToolHandler for GetRadarHandler {
    fn name(&self) -> &str { "get_radar" }
    fn description(&self) -> &str { "获取智识雷达推荐列表，返回按相关性排序的外部文章" }
    fn input_schema(&self) -> Value { definitions::get_radar_schema() }
    fn module(&self) -> &str { "radar" }
...
impl ToolHandler for AddToVaultHandler {
    fn name(&self) -> &str { "add_to_vault" }
    fn description(&self) -> &str { "将雷达中的文章保存到 Obsidian Vault" }
    fn input_schema(&self) -> Value { definitions::add_to_vault_schema() }
    fn module(&self) -> &str { "radar" }

    async fn handle(&self, args: Value, ctx: &Arc<AppContext>) -> Result<Value, BrainError> {
        let article_id = args.get("article_id")
            .and_then(|v| v.as_str())
            .filter(|s| !s.is_empty())
        ...
        let result = ctx.radar_service.add_to_vault(article_id, target_dir).await?;
...
impl ToolHandler for DismissRadarItemHandler {
    fn name(&self) -> &str { "dismiss_radar_item" }
    fn description(&self) -> &str { "忽略雷达条目，不再推荐" }
    fn input_schema(&self) -> Value { definitions::dismiss_radar_item_schema() }
    fn module(&self) -> &str { "radar" }
