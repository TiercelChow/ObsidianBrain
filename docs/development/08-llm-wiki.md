...
首个可运行纵切已经完成：书架 JSON 可无损迁移到正规表；每本书可建立独立知识库；Markdown 文件夹可扫描为带文件和行号引用的数据库章节实体；五个新页面已接通真实 API；研究任务、书籍配置文档和 Runtime Profile 均保存到 SQLite。自然语言检索目前采用轻量查询规整与数据库正文召回。

当前不会伪造 Agent 结果。PDF 只完成来源登记并明确标记为等待 Harness；知识问答只展示真实命中的证据；研究任务先保存为草稿。DeepSeek Harness ACP、PDF 版面提取、结构化论断/关系生成、审核、备份、导出和旧后端清理仍按本文后续阶段实施。
