...
知识库模块是 ObsidianBrain 的**洞察中枢**，通过分析用户的 Obsidian Vault 笔记结构和链接关系，提供 Obsidian 本身不具备的**知识健康度洞察**。它不重复 Obsidian 已有的搜索和编辑能力，而是帮助用户发现被遗忘的笔记、识别知识体系的核心节点、追踪知识增长趋势。
...
```
    Obsidian Vault（用户笔记）
           ...
           ▼
    ┌──────────────┐
    │   知识库      │  ← 分析笔记结构、链接关系、修改时间
    │  (本模块)     │
    └──────┬───────┘
           │
    ┌──────┴──────────────────────────┐
    ▼              ▼              ▼
  链接图谱分析   时间维度分析   目录/标签聚类
 (孤岛/枢纽)   (尘封/新生)    (领域分布)
```
...
| 洞察卡片 | 说明 | 用户价值 |
|---|---|---|
| 🏝️ **知识孤岛** | 没有任何其他笔记链接指向的笔记 | 发现被遗忘的笔记，决定是否建立链接或归档 |
| 🕸️ **知识枢纽** | 被最多其他笔记引用的核心笔记 | 识别知识体系的关键节点 |
| 📅 **尘封笔记** | 最久未修改的笔记 | 唤醒沉睡知识，决定是否需要更新 |
| 🌱 **新生知识** | 最近创建的笔记 | 追踪知识增长速度 |
| 📊 **知识领域** | 按文件夹聚类的笔记分布 | 了解知识结构是否均衡 |

### 1.3 与其他模块的关系

- **Obsidian REST API** (`infra/obsidian_client.rs`)：提供笔记列表、内容读取、元数据查询
- **时间线模块** (`core/timeline.rs`)：知识库洞察可引用时间线数据辅助分析
- **API 层** (`api/handlers/`)：将洞察操作暴露为标准 Tool API

---

## 2. 功能需求

### 2.1 知识孤岛 (Knowledge Islands)

**定义**：Vault 中没有任何其他笔记通过 `[[wikilink]]` 或 `[text](path)` 链接指向的笔记。

**展示**：
- 孤岛笔记总数
- 列表展示（路径 + 最后修改时间），按最后修改时间升序（最旧的在前）
- 点击可在 Obsidian 中打开
...
### 2.2 知识枢纽 (Knowledge Hubs)

**定义**：被最多其他笔记引用的笔记（入度最高的节点）。

**展示**：
- Top 10 枢纽笔记
- 每项显示：路径 + 被引用次数 + 引用它的笔记列表（折叠）
- 按引用次数降序

### 2.3 尘封笔记 (Dormant Notes)
...
|---|---|
| 洞察加载 | < 3s（100 篇笔记以内） |
| 分析精度 | 链接解析覆盖 `[[wikilink]]` 和 `[text](path)` 两种格式 |
| 排除范围 | 自动排除 `.obsidian/`、`templates/`、`.trash/`、非 `.md` 文件 |
