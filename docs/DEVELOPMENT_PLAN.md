...
> 基于 `docs/top_design.md` 和各模块设计文档拆分 | 最后更新：2026-08-17
...
**目标**：以 Obsidian 为权威数据源，完成短期待办、长期任务树、进展记录、任务视图和日历视图。

**设计文档**：

- [需求设计](requirement/09-task-management.md)
- [开发设计](development/09-task-management.md)

### 5.1 领域与存储基础

- [x] 任务模型、状态机、字段校验和错误类型
- [x] 多级树构建、循环检测、节点移动和聚合进度
- [x] 短期月文件与长期单任务文件 Markdown codec
- [x] SQLite migration 009 与可重建查询投影
- [x] Obsidian/SQLite trait 适配器和单元测试

### 5.2 服务与 Tool API

- [x] TaskService 创建、编辑、关闭、重开和归档
- [x] 子任务添加/移动和进展追加
- [x] path lock、文档版本乐观并发与同步恢复
- [x] 任务查询、游标分页和日历范围查询
- [x] 注册任务管理 Tool schema 与 handlers

### 5.3 任务视图

- [x] 前端路由、API、store、搜索和筛选
- [x] 桌面 master-detail 列表与详情
- [x] 手机列表、独立详情和底部 sheet 操作
- [x] 任务树、进展时间线和关闭说明流程
...
- [x] 桌面月历、跨日条与选中日日程
- [x] 手机紧凑月历与日程列表
- [x] 无障碍、reduced motion 和响应式验证

### 5.5 验收与可靠性

- [x] Vault → SQLite 全量重建
- [x] 文档版本冲突、重复 ID、损坏 YAML 和索引失败处理
- [ ] 10,000 个索引节点及 500 节点长期任务性能验证
- [ ] Rust 格式化、clippy、测试及前端类型检查、构建、E2E 全部通过
