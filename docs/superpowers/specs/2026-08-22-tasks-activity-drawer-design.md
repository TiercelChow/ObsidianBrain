...
### 13.1 属性胶囊下沉（修订 §12.3 应用位置）

- 删除主面板与抽屉头部的 kicker 胶囊行，标题上移至头部最上方。
- 状态 + 重要性胶囊并入事实行：置于「开始/结束」之前、同一行展示；「优先级」事实卡删除（与重要性胶囊重复）。
- 事实行由三列卡片网格改为单行 flex 布局（两个胶囊 + 两张日期卡片），窄屏自动换行。
...
- 收起把手改为贴抽屉左缘内侧的竖条：静止为 4px 细线，悬浮（及移动端常显）加宽至约 22px 并显示 ›；点击收起。push 槽总宽回到 `min(400px, 30vw)`，取消 24px 把手槽；Esc/遮罩关闭与「‹ {父任务名}」返回行保留。
