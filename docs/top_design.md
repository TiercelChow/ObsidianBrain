...
│  ┌───────────┐ ┌───────────┐ ┌──────────┐ ┌──────────────┐ │
│  │ API 网关   │ │ 工具注册表 │ │ 技能编排器│ │  事件总线    │ │
│  │ (Axum)    │ │ (Tool     │ │ (Skill   │ │  (EventBus)  │ │
│  │           │ │  Registry)│ │  Engine) │ │              │ │
│  └─────┬─────┘ └─────┬─────┘ └────┬─────┘ └──────┬───────┘ │
│        │             │            │               │         │
│  ┌─────┴─────────────┴────────────┴───────────────┴───────┐ │
│  │                    核心服务层                             │ │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌───────────┐  │ │
│  │  │ Memory   │ │ Timeline │ │ CodeRepo │ │Inspiration│  │ │
...
│  │                    基础设施层                             │ │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌───────────┐  │ │
│  │  │ Tantivy  │ │ Qdrant   │ │ SQLite   │ │ File      │  │ │
│  │  │ 全文索引  │ │ 向量存储  │ │ 元数据   │ │ Watcher   │  │ │
│  │  └──────────┘ └──────────┘ └──────────┘ └───────────┘  │ │
│  └─────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
         │                    │
...
LLM 调用 tool
    │
    ▼
API 网关接收 JSON-RPC / REST 请求
    │
    ▼
工具注册表查找对应 Tool Handler
    │
    ▼
Tool Handler 调用一个或多个 Service
    │
    ├─→ 需要 LLM？ ──→ LLM Client (OpenAI / Claude / Ollama)
    ├─→ 需要检索？ ──→ Tantivy + Qdrant 混合搜索
    ├─→ 需要 Git？ ──→ git2 读取仓库信息
    └─→ 需要写文件？ ──→ 安全写入 Obsidian Vault
    │
    ▼
组装结果，返回结构化 JSON
    │
...
| 全文搜索 | 笔记检索 | **Tantivy** + jieba-rs | Rust 原生搜索引擎，jieba 支持中文分词 |
| 向量存储 | 语义搜索 | **Qdrant** (Docker) | 高性能向量数据库，Rust 编写，REST/gRPC API |
| 元数据存储 | 配置/状态 | **SQLite** (rusqlite) | 轻量嵌入式数据库，无需额外进程 |
| 嵌入模型 | 向量化 | **OpenAI Embedding API** → 本地 ONNX | 初期快速验证，后期切换本地模型保隐私 |
| Git 操作 | 代码仓信息 | **git2** | libgit2 的 Rust 绑定，无需命令行调用 |
| 文件监控 | Vault 变更检测 | **notify** | 跨平台文件系统监控，实时感知笔记变更 |
| Markdown | 笔记解析 | **pulldown-cmark** + **gray_matter** | 解析 Markdown 正文 + YAML frontmatter |
| HTTP 客户端 | 外部请求 | **reqwest** | 异步 HTTP 客户端，支持连接池 |
| RSS 解析 | 信息聚合 | **feed-rs** | 支持 RSS/Atom，纯 Rust 实现 |
| LLM 调用 | AI 能力 | **reqwest** → OpenAI / Claude / Ollama | 统一封装，可切换 provider |
| 配置管理 | 应用配置 | **config** crate | 支持 TOML/YAML/ENV 多来源配置 |
| 日志 | 运行日志 | **tracing** + **tracing-subscriber** | 结构化日志，支持 span 追踪 |
| 定时任务 | 雷达轮询等 | **tokio-cron-scheduler** | 基于 Tokio 的轻量 cron 调度 |
| VSCode 集成 | 编辑器跳转 | `vscode://` URI / `code` 命令 | 标准协议 + 系统调用 |
